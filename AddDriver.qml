                ...
                Button {
                    ...
                    onClicked: {
                      ...
                      if (!imageError.visible && !firstnameError.visible && !lastnameError.visible && !phoneError.visible && !emailError.visible && !salaryError.visible && !hiredateError.visible) {
                       ...
                       console.log("Phone Number:", phonenumberField.text)
                       console.log("Email:", emailField.text)
                       console.log("Salary:", salary.text)
                       console.log("Hire Date:", hiredateField.text)
                       console.log("Image Path:", imagePath)

                       // Call the C++ method to add the driver
                      var result = db.addDriver(firstnameField.text, lastnameField.text, imagePath , addressField.text, phonenumberField.text, emailField.text, salary.text, hiredateField.text,0, "driver");
                          if (result === "success") { // Adjust based on your actual return type
                           displayMessage(Messages.getErrorMessage("driverAddedSuccessfully"), "green");
                              dialog.message="Successfull"
                              dialog.open()
                          } else {
                              dialog.message="Already Exists"
                              dialog.open()
//                           displayMessage(Messages.getErrorMessage("driverAlreadyExists"), "red");
                        }
                      }
                    }
                    contentItem: Text {
